...
class PurchaseIntegrationTest {
    ...
    void purchaseFlow_ShouldSucceed() throws Exception {
        ...
        String adminToken = objectMapper.readTree(adminLoginResult.getResponse().getContentAsString()).get("token")
                .asText();

        // 3. Add Sweet (as Admin)
        Sweet sweet = new Sweet(null, "Test Sweet", "Test Category", BigDecimal.valueOf(10.0), 100, "Description",
                "http://url");
        ...
        Sweet savedSweet = objectMapper.readValue(addSweetResult.getResponse().getContentAsString(), Sweet.class);
        ...
        String userToken = objectMapper.readTree(userLoginResult.getResponse().getContentAsString()).get("token")
                .asText();
        ...
        assertEquals(BigDecimal.valueOf(50.0), purchase.getTotalPrice());
    }
}
